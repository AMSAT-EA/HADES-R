...
#define PACKET_ADDRESS_SATELLITE_ID				0xF // 8 FOR HADES-D, 9 FOR GENESIS-M, A FOR HYDRA-T,
													// B for MARIA-G, C FOR UNNE-1,    D for HADES-R,
													// E for HYDRA-W, F for HADES-ICM
...
#define PACKET_TYPE_COMMAND                 	(0x00 | PACKET_ADDRESS_SATELLITE_ID)
...
#define PACKET_TYPE_PAYLOAD_DATA_TERA    		(0x70 | PACKET_ADDRESS_SATELLITE_ID)
...
#define PACKET_TYPE_PAYLOAD_DATA_UC3M	  		(0xD0 | PACKET_ADDRESS_SATELLITE_ID)
...
#define TERA_MAX_FRAMES			 		8
#define TERA_PAYLOAD_DATA_SIZE			32
...
#define PAYLOAD_DATA_SIZE               128 // UC3M
...
// esta es la estructura para la payload UC3M - No borrar, hace falta para el tm

typedef struct st_payload_data_packet {
    ...
    // temperatura cuando se toma la muestra
    int8_t temp_sample;
...
} __attribute__((packed)) payload_data_packet;



typedef struct st_payload_tera_data_packet {
    ...
    uint8_t data[TERA_PAYLOAD_DATA_SIZE];

    uint16_t checksum;

} __attribute__((packed)) payload_tera_data_packet;
